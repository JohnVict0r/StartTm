...
from eventos.models import EventoCategoria, Evento, InscricaoSolicitacao,Perfil


def index(request):
    return render(request, 'eventos/index.html', {'eventos': Evento.objects.all(),'perfil_logado' : get_perfil_logado(request)})
...
def exibir_evento(request, evento_id):
    ...
    return render(request, 'eventos/eventos.html', {"evento": evento , "evento_categoria": evento_categoria ,"inscrito": i, 'perfil_logado' : get_perfil_logado(request)})
